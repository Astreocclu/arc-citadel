...
impl Needs {
    ...
    /// - Rest: reaches critical (~0.8) in ~80 active ticks
    /// - Food: reaches critical in ~160 ticks (eat less often than rest)
    /// - Social: slow buildup, ~260 ticks to critical
    /// - Purpose: slowest, ~400 ticks to critical
    ...
    pub fn decay(&mut self, dt: f32, is_active: bool) {
        ...
        // Target: reach 0.8 around tick 800 from starting value of ~0.5
        // This means ~0.3 increase over 800 ticks = 0.000375/tick
        // Using 0.001 for rest (fastest), scaling down for others
        // Hierarchy: rest > food > social > purpose
        self.rest += 0.001 * dt * activity_mult;    // 1000 ticks to max (faster when active)
        self.food += 0.0008 * dt;                   // 1250 ticks to max
        self.social += 0.0003 * dt;                 // 3333 ticks to max (slow build per E5)
        self.purpose += 0.0002 * dt;                // 5000 ticks to max (slowest)
