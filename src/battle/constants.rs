...
// Approach: armies marching toward each other (sped up but not too fast)
// Combat: units actively fighting (interesting, normal speed)
// Rout: one side fleeing (resolution, medium speed)
pub const TIME_SCALE_APPROACH: f32 = 1.0;   // 1 tick = 1 sim-second while marching (real time)
pub const TIME_SCALE_COMBAT: f32 = 1.0;     // 1 tick = 1 sim-second during combat
pub const TIME_SCALE_ROUT: f32 = 1.5;       // 1 tick = 1.5 sim-seconds during rout
