...
pub fn tick(polity: &Polity, _world: &AggregateWorld, _year: u32) -> Vec<EventType> {
    ...
    // Access goblin-specific state
    if let Some(state) = polity.goblin_state() {
...
mod tests {
    ...
    fn create_test_polity() -> Polity {
        Polity {
            id: PolityId(1),
            name: "Test Goblin Polity".to_string(),
    ...
    #[test]
    fn test_goblin_state_accessor() {
        let polity = create_test_polity();
        let state = polity.goblin_state();
