...
use crate::aggregate::polity::{DecisionType, GrudgeReason, TreatyTerms};
...
impl HistoryLog {
    ...
    pub fn events_for_polity(&self, polity_id: u32) -> impl Iterator<Item = &Event> {
        self.events
            .iter()
            .filter(move |e| e.participants.contains(&polity_id))
    }
}
